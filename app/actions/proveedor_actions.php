...
if ($action === 'create') {
    ...
    if (empty($titulo)) { header('Location: ../pages/proveedor_panel.php?err=1'); exit; }
    ...
    header('Location: ../pages/proveedor_panel.php?' . ($ok ? 'added=1' : 'err=1'));
    exit;
}
...
if ($action === 'update') {
    ...
    if ($id <= 0 || empty($titulo)) { header('Location: ../pages/proveedor_panel.php?err=1'); exit; }
    ...
    if (!$row) { header('Location: ../pages/proveedor_panel.php?err=1'); exit; }

    if ($nueva_img && $row['imagen']) {
        $old = dirname(__DIR__, 2) . '/uploads/' . $row['imagen'];
    ...
    mysqli_stmt_close($stmt);
    header('Location: ../pages/proveedor_panel.php?' . ($ok ? 'ok=1' : 'err=1'));
    exit;
}
...
if ($action === 'delete') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id <= 0) { header('Location: ../pages/proveedor_panel.php?err=1'); exit; }
    ...
    header('Location: ../pages/proveedor_panel.php?' . ($ok ? 'del=1' : 'err=1'));
    exit;
}
...
if ($action === 'change_password') {
    ...
    if (empty($current) || empty($new)) {
        header('Location: ../pages/proveedor_panel.php?tab=perfil&err=1');
        exit;
    }
    ...
    if ($user && password_verify($current, $user['password'])) {
        ...
        if (mysqli_stmt_execute($stmt_upd)) {
            ...
            header('Location: ../pages/proveedor_panel.php?tab=perfil&ok=1');
            exit;
        }
    }
...
header('Location: ../pages/proveedor_panel.php');
exit;
