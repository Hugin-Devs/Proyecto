...
include_once __DIR__ . '/get_lists.php';
...
if ($search_f !== '') {
    $where_clauses[] = "(titulo LIKE ? OR descripcion LIKE ?)";
...
$count_query = "SELECT COUNT(*) as total FROM servicios WHERE $where_sql";
...
if (mysqli_num_rows($resultado) > 0) {
    while ($s = mysqli_fetch_assoc($resultado)) {
        ...
        $imagen         = !empty($s['imagen']) ? 'uploads/' . htmlspecialchars($s['imagen']) : '';
